...
- **Time-series**
  ...
  > **The one question that actually decides SQL vs. NoSQL:** does this write need to be correct *together* with another write, or does this read need to be correct *together* with another read? Booking a flight touches multiple related facts that must all be true at once — decrement seat inventory, insert the booking, charge the payment — and if the charge fails, the seat decrement has to roll back too, or you've sold a seat nobody paid for. That's a transaction, and it's exactly what SQL guarantees; you also constantly need relational questions answered ("this user's bookings joined with payment status"), which means `JOIN`s across `users`/`bookings`/`payments` tables. A hotel catalogue is the opposite: reading one listing never needs to lock or join against another, and different hotels genuinely have different shapes of data (a boutique hotel has `pet_friendly`, a business hotel has `conference_rooms`) — forcing that into rigid SQL columns means a table full of mostly-NULL fields. NoSQL lets each document stand alone with its own shape, and scales by just adding machines, each independently serving whichever documents they hold, with no cross-machine transaction to coordinate. Same logic explains why **sessions** sit in NoSQL/Redis (each one is independent, high-volume, ephemeral) while **bookings and payments** sit in SQL (interdependent, must be transactionally consistent, permanent record).
...
- **Horizontal scaling**
  ...
  > **Why statelessness is the prerequisite, not a nice-to-have:** a load balancer can route a user's requests to a *different* instance every time. If an instance keeps request-specific state in its own local memory or disk (a session, a cart, a file mid-upload), a request that lands on the wrong instance breaks. Stateless design pushes all of that into a shared external store — Redis/DB for session state, a signed JWT the client carries instead of a server-side session, S3 instead of local disk — so any instance can serve any request. That interchangeability is what lets you freely add, remove, or replace instances (near-linear scaling) and also what makes rolling deploys safe, since no single instance is the sole holder of anything important.
...
- **Short-circuiting**
  Stop an agent pipeline the moment a step already satisfies the task, instead of running every planned hop to completion. It trades completeness of the reasoning chain for cost, latency, and safety.

  > **How it applies to agentic design:** the same idea as boolean short-circuit evaluation (`a && b` skips `b` once `a` is false), applied to orchestration. It shows up at several points in an agent pipeline: a cache hit or guardrail check that skips an LLM call entirely; a verifier that fails early in a planner → retriever → verifier → synthesiser chain, halting before downstream agents burn tokens on doomed input; a tool-calling loop that stops as soon as a result meets the success criteria rather than running to the max-turn budget; an ensemble of parallel agents that returns once N of M already agree, without waiting on the rest; and a policy filter that blocks unsafe input before any tool or model runs at all, so a reasoning chain never gets the chance to justify an irreversible action. Implemented via confidence thresholds between steps, conditional edges in an orchestrator graph, or a circuit breaker that stops calling a repeatedly failing dependency and fails fast rather than cascading. The design skill is picking the right checkpoint to evaluate "is this good enough to stop here" — too early loses correctness, too late loses the cost/latency/safety benefit.
...
- **Circuit breakers**
  ...
  > **How it actually works:** a circuit breaker wraps a dependency call and tracks its failure rate, moving through three states like the electrical device it's named after. **Closed** (normal) — every call goes through, failures are counted. **Open** — once failures cross a threshold, the breaker trips: for a cooldown window, every call fails *instantly* without even touching the dependency, no timeout wait required. **Half-open** — after the cooldown, a small number of trial requests are let through; if they succeed, the breaker closes again; if they fail, it reopens for another cooldown. The specific failure mode it targets: a timeout alone still makes every caller wait out the full deadline on every request to a dead dependency, tying up a thread or connection each time — under load, that alone can exhaust the caller's own pool and take down a perfectly healthy service. Failing fast is what breaks that cascade; the cooldown is what gives the failing dependency room to actually recover instead of getting hit by retry traffic the instant it starts responding again.
...
- **Chaos testing**
  ...
  > **Reliability and availability are not the same target, so each technique above earns its place by protecting one of them.** Availability is *whether* the system responds — measured in nines, and won by redundancy: multi-AZ, multi-region failover, health checks, and load balancing all exist to route traffic around something that's down. Reliability is *whether the response is correct and no data is lost*, even mid-failure — won by containment: timeouts, circuit breakers, retries-with-backoff, and dead letter queues all exist to stop one failure from corrupting or cascading into others. A system can be highly available and unreliable (always responds fast, sometimes with the wrong answer or silently dropped data) or reliable and unavailable (correct whenever it responds, but down often) — naming which techniques buy you which property is what separates a checklist from a design. Backup & restore is reliability's version of a safety net (bounded by RPO — how much data you can afford to lose); multi-region failover is availability's (bounded by RTO — how long you can afford to be down).
...
- **Active-active architecture**
  ...
  > **Active-active vs. active-passive, and what it actually costs you:** active-passive (the "multi-region failover" in §10) keeps one region live and a standby idle or replicating; losing the primary means *detecting* the failure, *promoting* the standby, and *repointing* traffic (DNS/global LB) — real downtime measured by that RTO. Active-active removes the failover step itself: every region is already serving live traffic, so losing one just means the global load balancer stops routing there and the surviving regions absorb the rest — nothing to detect, promote, or repoint. The price is on the data layer, not the network: with two-plus regions accepting *writes* to the same logical data, you now have concurrent writers, and something has to reconcile them. The three ways out are (1) partition writes by region so they never overlap — the AMER/EMEA/APJ pattern from the travel-agent design, where each region owns its own users, (2) conflict-free replicated data types or last-writer-wins for data that genuinely gets written from multiple regions, or (3) accept eventual consistency and resolve conflicts in application logic. There's a capacity cost too: every live region must be sized for real traffic (not idling as a standby), and — for the failover benefit to actually hold — the survivors need enough spare headroom to absorb a failed region's load, not just their own.
...
1. **Define the problem**
   Clarify scope, users, and what you are explicitly not building.

2. **Gather requirements**
   Separate functional capabilities from quantified quality attributes.

3. **Estimate scale**
   Users, RPS, storage, and bandwidth, with your assumptions stated.

4. **Design the high-level architecture**
   The major blocks and the path a request takes.

5. **Design data storage and services**
   Pick stores per access pattern and define service boundaries.

6. **Explain end-to-end data flow**
   Trace one real request through every component.

7. **Address scalability**
   Name the bottleneck first, then the technique that removes it.

8. **Address reliability**
   Describe what happens when each dependency fails.

9. **Address security and governance**
   Authentication, authorisation, data protection, compliance.

10. **Discuss tradeoffs and future enhancements**
    What you gave up, and what you would do next.
