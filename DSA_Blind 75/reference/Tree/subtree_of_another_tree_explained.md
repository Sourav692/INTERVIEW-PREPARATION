...
`root = [3,4,5,1,2,…,0]` vs `sub = [4,1,2]`

| Step | Current `root` node | `same_shape(root, sub)?` | Next action |
| ---- | ------------------- | ------------------------ | ----------- |
| 1 | `3` | No | search left |
| 2 | `4` | No (left child of 4 is `1` in sub but `1` with child `0` in root) | search left |
| 3 | `1` | No | search left & right |
| 4 | `2` | No | search left & right |
| 5 | `5` | No | search left & right |
| 6 | `0` | No | exhausted |
| — | `None` reached | — | **`False`** |
...
### Step-by-step trace — false case (extra `0` under `1`)

The subtree at `4` in the false `root` has shape `[4,1,2]` but node `1` has a child `0`, so:

```
ser(4_with_0) = "^4 ^1 ^0 # # ^2 # #"
```

`ser(sub)` = `"^4 ^1 # # ^2 # #"` — **not** a substring because of the extra `^0` branch.
